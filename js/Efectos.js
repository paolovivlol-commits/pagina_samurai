// esperar a que cargue todo antes de ejecutar
window.addEventListener('load', () => {
  ...
  window.addEventListener('scroll', () => {
    if (fotofondo) {
      fotofondo.style.transform = `scale(1.05) translateY(${window.scrollY * 0.3}px)`;
    }
  });
  ...
  if (parallaxImg) {
    window.addEventListener('scroll', () => {
      ...
      if (rect.top < window.innerHeight && rect.bottom > 0) {
        parallaxImg.style.transform = `translateY(${-rect.top / window.innerHeight * 60}px)`;
      }
    });
  }
  ...
  if (imgSamurai) {
    window.addEventListener('scroll', () => {
      ...
      const rect = seccion.getBoundingClientRect();
      if (rect.top < window.innerHeight && rect.bottom > 0) {
        imgSamurai.style.transform = `scale(1.06) translateY(${-rect.top / window.innerHeight * 30}px)`;
      }
    });
  }
      ...
      if (entry.isIntersecting) {
        ...
        if (!isNaN(numero)) {
          ...
          const intervalo = setInterval(() => {
            ...
            if (cuenta >= numero) { cuenta = numero; clearInterval(intervalo); }
